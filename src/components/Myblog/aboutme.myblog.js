import React from "react";
import Ssection6 from "./style.aboutme.myblog";
const Section3_aboutme = () => {
  return (
    <Ssection6>
      <div className="Section6">
        <div className="img6 ">
          <img src="https://company.senspark.com/static/media/Hai-Hoang.9e087eb5e142c2225fcf.png" />
        </div>
        <div className="text6">
          <h1 className="title6">My blog</h1>
          <div className="content6">
            I'll let you in on some trade secrets and share my top tips on how
            to successfully promote your brand online.
          </div>
          <button className="button6"> Read blog</button>
        </div>
      </div>
    </Ssection6>
  );
};

export default Section3_aboutme;
